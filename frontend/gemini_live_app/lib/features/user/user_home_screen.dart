...
import 'package:vision_aid_app/core/services/command_router.dart';
...
class _UserHomeScreenState extends ConsumerState<UserHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Listen to user mode changes to navigate to vision screen
    ref.listen<AppMode>(userModeProvider, (previous, next) {
      if (next == AppMode.vision) {
        Navigator.pushNamed(context, '/vision').then((_) {
          // When returning, reset to idle
                  ...
                  if (!routeState.isActiveNavigation)
                    Positioned(
                      ...
                      child: FloatingActionButton.large(
                        onPressed: () => ref.read(commandRouterProvider).routeCommand("alert guardian"),
                        backgroundColor: Colors.red,
                        child: const Text("SOS", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 24)),
                      ),
                    ),
                ],
              ),
