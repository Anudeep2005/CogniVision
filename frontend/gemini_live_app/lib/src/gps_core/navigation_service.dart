...
class NavigationService {
  ...
  Future<RouteState?> navigateTo(String destination) async {
    final String apiKey = dotenv.env['GOOGLE_MAPS_API_KEY'] ?? dotenv.env['MAPS_API_KEY'] ?? '';

    try {
      _voiceService.speak("Calculating walking route to $destination.");

      // 1. Get Current Location
      bool serviceEnabled = await Geolocator.isLocationServiceEnabled();
      if (!serviceEnabled) {
        await _voiceService.speak("Location services are disabled. Please enable them.");
        return null;
      }
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          await _voiceService.speak("Location permissions are denied.");
          return null;
        }
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'OK') {
          ...
          // 3. Read Instructions
          await _voiceService.speak("Route found. Say start to begin navigation.");
          ...
          await _voiceService.speak("I could not find a walking route to $destination.");
        }
      } else {
        await _voiceService.speak("Network error while fetching directions.");
      }
    } catch (e) {
      debugPrint("Navigation Error: $e");
      await _voiceService.speak("An error occurred while setting up navigation.");
      return null;
    }
    return null;
  }
  ...
  Future<void> updateLiveLocation(Position currentPosition, RouteState routeState, Function(RouteState) updateState) async {
    ...
    if (distance <= 15.0) { 
      if (routeState.currentStepIndex == routeState.steps.length - 1) {
        await _voiceService.speak("You have arrived at your destination.");
        // Reset navigation
